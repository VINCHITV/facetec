...
export class LivenessCheckProcessor implements FaceTecFaceScanProcessor {
    ...
    //
    public async processSessionResultWhileFaceTecSDKWaits(sessionResult: FaceTecSessionResult, faceScanResultCallback: FaceTecFaceScanResultCallback) {
        ...
        const Base_url = "/Users/kaguya/Downloads/facetecCurlReq";
        const lowQualtiyAutditTrailPath: string = Base_url + "/lowQualityAuditTrail.txt";
        const auditTrail: string = Base_url + "/auditTrail.txt";
        const facescan: string = Base_url + "/facescan.txt";

        // import  fs from "fs";
        // import FormData from "form-data";

        const faceScanformData = new FormData();
        const auditTrailformData = new FormData();
        const lowQualityAuditTrailformData = new FormData();
        // form.append('uploads', fs.readFileSync('main.js'), 'rot.js');
        if (parameters.faceScan !== null){
            faceScanformData.append('file', new Blob([parameters.faceScan], { type: "text/plain" }) );
            faceScanformData.append('name', "facescan.txt",);
        }
        if (parameters.auditTrailImage !== null){
            auditTrailformData.append('file', new Blob([parameters.auditTrailImage], { type: "text/plain" }) );
            auditTrailformData.append('name', "auditTrail.txt",);
        }
        if (parameters.lowQualityAuditTrailImage !== null){
            lowQualityAuditTrailformData.append('file', new Blob([parameters.lowQualityAuditTrailImage], { type: "text/plain" }) );
            lowQualityAuditTrailformData.append('name', "lowQualityAuditTrail.txt",);
        }

        fetch('http://localhost:3456/upload', {
            method: 'POST',
            body: faceScanformData
        })
            .then(response => {
                console.log("faceScan Uploaded Sucessfully")
                // handle success
            })
            .catch(error => {
                console.log("faceScan Uploaded failed")
                // handle error
            });

        fetch('http://localhost:3456/upload', {
            method: 'POST',
            body: auditTrailformData
        })
            .then(response => {
                console.log("auditTrail Uploaded Sucessfully")
                // handle success
            })
            .catch(error => {
                console.log("auditTrail Uploaded failed")
                // handle error
            });

        fetch('http://localhost:3456/upload', {
            method: 'POST',
            body: lowQualityAuditTrailformData
        })
            .then(response => {
                console.log("lowQuality Audit trail Uploaded Sucessfully")
                // handle success
            })
            .catch(error => {
                console.log("lowQuality Audit trail Uploaded failed")
                // handle error
            });
